...
// arpBPFFilter returns compiled BPF instructions that match ARP packets on
// untagged Ethernet and single-tag VLAN Ethernet (802.1Q / 802.1ad).
func arpBPFFilter() ([]bpf.RawInstruction, error) {
	// Filter:
	//   - accept untagged Ethernet frames with EtherType == ARP (0x0806)
	//   - accept single-tag VLAN frames where outer EtherType is 0x8100 or 0x88a8
	//     and the encapsulated EtherType at offset 16 is ARP (0x0806)
	return bpf.Assemble([]bpf.Instruction{
		bpf.LoadAbsolute{Off: 12, Size: 2},                                      // load outer EtherType/TPID at offset 12
		bpf.JumpIf{Cond: bpf.JumpEqual, Val: 0x0806, SkipTrue: 6, SkipFalse: 0}, // untagged ARP -> accept
		bpf.JumpIf{Cond: bpf.JumpEqual, Val: 0x8100, SkipTrue: 2, SkipFalse: 0}, // 802.1Q VLAN -> check inner EtherType
		bpf.JumpIf{Cond: bpf.JumpEqual, Val: 0x88a8, SkipTrue: 1, SkipFalse: 0}, // 802.1ad/QinQ outer tag -> check inner EtherType
		bpf.RetConstant{Val: 0},            // reject non-ARP, non-VLAN-tagged frames
		bpf.LoadAbsolute{Off: 16, Size: 2}, // load encapsulated EtherType after single VLAN tag
		bpf.JumpIf{Cond: bpf.JumpEqual, Val: 0x0806, SkipTrue: 1, SkipFalse: 0}, // inner ARP -> accept
		bpf.RetConstant{Val: 0},      // reject other encapsulated protocols
